...
-- Create enhanced function that handles onboarding data
    ...
    CASE user_type
      ...
      WHEN 'cohost' THEN
        IF cohost_data IS NOT NULL AND cohost_data != 'null'::jsonb THEN
          INSERT INTO public.cohosts (user_id, skills, availability, hourly_rate, min_rate, max_rate)
          VALUES (
            NEW.id,
            ARRAY(SELECT jsonb_array_elements_text(cohost_data->'skills')),
            cohost_data->>'availability',
            (cohost_data->>'hourlyRate')::numeric,
